...
const Addresses =(props) => {
  ...
  const handleCheck = (event) => {
    ...
    setChecked(checked.includes(value) ? checked.filter(c => c !== value) : [value]);    
  };
  ...
  props.addresses.map((address) => (
    ...
    >
      <FormControlLabel
        ...
        control={<Checkbox onChange={e => handleCheck(e)} checked ={checked.includes(String(address.id))} name={address.id}/>}
        label={address.name}
      />
    </AccordionSummary>
    <AccordionDetails>
      ...
      <Typography color="textSecondary">{address.name}</Typography>
          <Typography color="textSecondary">{address.address1}, {address.address2},</Typography>
          <Typography color="textSecondary">{address.city},{address.state}</Typography>
...
const Checkout = () => {
  ...
  function getStepContent(step) {
    switch (step) {
      case 0:{
        ...
        return <Addresses addresses={addresses} checked={checked} setChecked={setChecked}/>;
      ...
      case 2:
        ...
        cartItems.map((prod) =>(
          items.push({ "quantity": prod.quantity,
          "product": prod.id,
          "price": prod.price_id})
        ))
        var order = {
  ...
  const handleNext = () => {
    ...
    if(!createAddress){
      ...
      if(name.length == 0 ||  mobile.length !=13 || addressline1.length == 0 || city.length == 0 || state.length == 0 || pincode.length == 0)
    {
      ...
      if(addressId){
        axios.put(`${address_create_url}${addressId}/`, {        
